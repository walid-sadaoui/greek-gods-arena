...
export const FightOpponent: React.FC<FightOpponentProps> = ({
  ...
  useEffect(() => {
    if (opponentTurn) {
      setAnimationState(["start", "attack"]);
    } else {
      if (remainingHealth === 0) {
        setAnimationState(["start", "damage", "death"]);
      } else {
        setAnimationState(["start", "damage"]);
      }
    }
  }, [opponentTurn, remainingHealth]);
  ...
  return (
    <div className={`flex flex-col p-4 ${isEnemy ? "items-end" : ""}`}>
      ...
      />
      <div
        className={`flex gap-4 flex-col ${isEnemy ? "items-end" : "items-start"} justify-around`}
      >
